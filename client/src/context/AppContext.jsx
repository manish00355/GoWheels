...
export const AppProvider = ({children})=>{ 
    ...
    const [pickupDate, setPickupDate] = useState("");
     ...
     const fetchUser = async()=>{
      ...
      // function to fetch all cars from server(backend)
     ...
     // useEffect to retrieve the token from local storage when the component mounts
     useEffect(()=>{
        const token = localStorage.getItem('token')
        setToken(token)
        fetchCars() // fetch cars when app loads
